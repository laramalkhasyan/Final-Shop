  ...
  */
  ...
  _setupAttrs(attrs) {
    ...
    Object.keys(attrs).forEach(attr => {
      ...
      if (!isAssociation) {
        this.attrs[attr] = attrs[attr]; // define plain getter/setters for non-association keys
  ...
  _validateAttrs(attrs) {
    Object.keys(attrs).forEach(key => {
      let value = attrs[key]; // Verify attrs passed in for associations are actually associations
  ...
  _validateForeignKeyExistsInDatabase(foreignKeyName, foreignKeys) {
    if (Array.isArray(foreignKeys)) {
      let association = Object.keys(this.hasManyAssociations).map(key => this.hasManyAssociations[key]).find(association => association.getForeignKey() === foreignKeyName);
      let found;
      ...
      let association = Object.keys(this.belongsToAssociations).map(key => this.belongsToAssociations[key]).find(association => association.getForeignKey() === foreignKeyName);
      let found;
