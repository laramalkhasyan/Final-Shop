...
function (_Association) {
  ...
  _createClass(BelongsTo, [{
    ...
    value: function addMethodsToModelClass(ModelClass, key) {
      ...
      modelPrototype.belongsToAssociations = Object.assign(modelPrototype.belongsToAssociations, associationHash); // Add to target's dependent associations array
...
function (_Association) {
  ...
  _createClass(HasMany, [{
    ...
    value: function addMethodsToModelClass(ModelClass, key) {
      ...
      modelPrototype.hasManyAssociations = Object.assign(modelPrototype.hasManyAssociations, associationHash); // Add to target's dependent associations array
...
function () {
  ...
  _createClass(BaseRouteHandler, [{
    ...
    value: function getModelClassFromPath(fullPath) {
      ...
      var modelName = dasherize(camelize(this._container.inflector.singularize(lastPath)));
      return modelName;
    }
  }, {
  ...
  _createClass(Model, [{
    ...
    */
    ...
    value: function _setupAttrs(attrs) {
      ...
      this._validateAttrs(attrs); // Ensure fks are there


      this.fks.forEach(function (fk) {
        _this2.attrs[fk] = attrs[fk] !== undefined ? attrs[fk] : null;
      });
      Object.keys(attrs).forEach(function (attr) {
        var isAssociation = _this2.associationKeys.has(attr) || _this2.associationIdKeys.has(attr);

        if (!isAssociation) {
          _this2.attrs[attr] = attrs[attr]; // define plain getter/setters for non-association keys

          _this2._definePlainAttribute(attr);
        }
      });
    }
    /**
    ...
    value: function _setupRelationships(attrs) {
      ...
      Object.keys(attrs).forEach(function (attr) {
        ...
        var isFk = _this3.associationIdKeys.has(attr) || _this3.fks.includes(attr);

        var isAssociation = _this3.associationKeys.has(attr);

        if (isFk) {
          if (value !== undefined && value !== null) {
            _this3._validateForeignKeyExistsInDatabase(attr, value);
          }

          _this3.attrs[attr] = value;
        }
    ...
    value: function _validateAttrs(attrs) {
      ...
      Object.keys(attrs).forEach(function (key) {
        var value = attrs[key]; // Verify attrs passed in for associations are actually associations

        {
          if (_this4.associationKeys.has(key)) {
        ...
        {
          if (_this4.associationIdKeys.has(key)) {
            if (key.endsWith("Ids")) {
              var isArray = Array.isArray(value);

              var _isNull = value === null;

              assert(isArray || _isNull, "You're trying to create a ".concat(_this4.modelName, " model and you passed in \"").concat(value, "\" under the ").concat(key, " key, but that key is a foreign key for a HasMany relationship. You must pass in an array of ids or null."));
            }
          }
...

        {
          ...
          if (isModelOrCollection || _isArrayOfModels) {
            ...
            assert(_this4.associationKeys.has(key), "You're trying to create a ".concat(_this4.modelName, " model and you passed in a ").concat(modelOrCollection.toString(), " under the ").concat(key, " key, but you haven't defined that key as an association on your model."));
          }
        }
      });
    }
